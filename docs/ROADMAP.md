...
- [x] Этап 0 — стабилизация основы 0.1.x (завершён 17 сентября 2026).
...
- Воспроизводимый Blueprint showcase smoke test на чистом UE 5.8-проекте.
...
- Smoke test создаёт граф с exec/data connections, компилирует, сохраняет, повторно открывает и инспектирует его без ручного вмешательства.
...
Ближайший шаг — этап 0.5, потому что проведённая Blueprint-проверка обнаружила невозможность исправить ошибочную ноду в существующем графе. Затем порядок 1 → 4 остаётся обязательным: каталог нод нельзя считать универсальным без типизированного codec и безопасной модели вызовов. После этапа 4 Enhanced Input используется как первый доказательный вертикальный сценарий. Backend SDK формализуется на основании опыта минимум двух различающихся подсистем, чтобы интерфейс не проектировался абстрактно.
